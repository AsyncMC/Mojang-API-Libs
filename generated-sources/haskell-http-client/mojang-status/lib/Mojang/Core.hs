...
-- | 
data MojangConfig = MojangConfig
...
-- | display the config
instance P.Show MojangConfig where
  show c =
    T.printf
...
-- | constructs a default MojangConfig
--
-- configHost:
--
...
-- @"mojang/0.1.0.0"@
--
newConfig :: IO MojangConfig
newConfig = do
    logCxt <- initLogContext
    return $ MojangConfig
        { configHost = "https://status.mojang.com"
        , configUserAgent = "mojang/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => MojangConfig -> auth -> MojangConfig
addAuthMethod config@MojangConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: MojangConfig -> IO MojangConfig
...
withStderrLogging :: MojangConfig -> IO MojangConfig
...
withNoLogging :: MojangConfig -> MojangConfig
...
data MojangRequest req contentType res accept = MojangRequest
  { rMethod  :: NH.Method   -- ^ Method of MojangRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of MojangRequest
  , rParams   :: Params -- ^ params of MojangRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (MojangRequest req contentType res accept) NH.Method
rMethodL f MojangRequest{..} = (\rMethod -> MojangRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (MojangRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f MojangRequest{..} = (\rUrlPath -> MojangRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (MojangRequest req contentType res accept) Params
rParamsL f MojangRequest{..} = (\rParams -> MojangRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (MojangRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f MojangRequest{..} = (\rAuthTypes -> MojangRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => MojangRequest req contentType res accept -> param -> MojangRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: MojangRequest req contentType res accept -> param -> MojangRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: MojangRequest req contentType res accept -> param -> MojangRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method 
          ...
          -> MojangRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = MojangRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: MojangRequest req contentType res accept -> [NH.Header] -> MojangRequest req contentType res accept
setHeader req header =
  req `removeHeader` P.fmap P.fst header &
  L.over (rParamsL . paramsHeadersL) (header P.++)

removeHeader :: MojangRequest req contentType res accept -> [NH.HeaderName] -> MojangRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => MojangRequest req contentType res accept -> MojangRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => MojangRequest req contentType res accept -> MojangRequest req contentType res accept
_setAcceptHeader req =
    case mimeType (P.Proxy :: P.Proxy accept) of 
        Just m -> req `setHeader` [("accept", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["accept"]

setQuery :: MojangRequest req contentType res accept -> [NH.QueryItem] -> MojangRequest req contentType res accept
setQuery req query = 
  req &
  L.over
...
addForm :: MojangRequest req contentType res accept -> WH.Form -> MojangRequest req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: MojangRequest req contentType res accept -> NH.Part -> MojangRequest req contentType res accept
...
_setBodyBS :: MojangRequest req contentType res accept -> B.ByteString -> MojangRequest req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: MojangRequest req contentType res accept -> BL.ByteString -> MojangRequest req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => MojangRequest req contentType res accept -> P.Proxy authMethod -> MojangRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    :: MojangConfig
    -> a
    -> MojangRequest req contentType res accept
    -> IO (MojangRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> MojangConfig
  -> IO (MojangRequest req contentType res accept)
_applyAuthMethods req config@(MojangConfig {configAuthMethods = as}) =
  foldlM go req as
  where
