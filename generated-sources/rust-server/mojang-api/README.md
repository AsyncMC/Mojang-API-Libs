...
- API version: 2020-06-05.0.0
- Build date: 2020-06-06T05:01:52.635789700-03:00[America/Sao_Paulo]
...
```
cargo run --example client GetOrdersStatistics
cargo run --example client FindUniqueIdsByName
cargo run --example client GetNameHistory
cargo run --example client GetUniqueIdByName
cargo run --example client CheckSecurityStatus
cargo run --example client ListPendingSecurityQuestions
cargo run --example client SendSecurityQuestionAnswers
cargo run --example client ChangePlayerSkin
cargo run --example client ResetPlayerSkin
cargo run --example client UploadPlayerSkin
```
...
* Add `openapi_client = {version = "2020-06-05.0.0", path = "openapi_client"}` under `[dependencies]` in the root `Cargo.toml`.
