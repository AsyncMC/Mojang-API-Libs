...
impl<F, C> Api<C> for Client<F> where
            ...
            match response.status().as_u16() {
                200 => {
                    ...
                    Box::new(
                        ...
                        .map(move |body|
                            GetBlockedServerHashesResponse::AListOfSHA(body)
                        )
    ...
    fn get_player_profile(&self, param_stripped_uuid: String, param_unsigned: Option<bool>, context: &C) -> Box<Future<Item=GetPlayerProfileResponse, Error=ApiError>> {

        // Query parameters
        let query_unsigned = param_unsigned.map_or_else(String::new, |query| format!("unsigned={unsigned}&", unsigned=query.to_string()));


        let uri = format!(
            "{}/session/minecraft/profile/{stripped_uuid}?{unsigned}",
            self.base_path, stripped_uuid=utf8_percent_encode(&param_stripped_uuid.to_string(), ID_ENCODE_SET),
            unsigned=utf8_percent_encode(&query_unsigned, QUERY_ENCODE_SET)
        );
                    ...
                    Box::new(
                        ...
                        .and_then(|body| str::from_utf8(&body)
                                             ...
                                             .and_then(|body|
                                                 ...
                                                 serde_json::from_str::<models::PlayerProfile>(body)
                                                     .map_err(|e| e.into())

                                             ))
                        .map(move |body|
                            GetPlayerProfileResponse::ThePlayerProfileAndAllProperties(body)
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                400 => {
                    ...
                    Box::new(
                        ...
                        .and_then(|body| str::from_utf8(&body)
                                             ...
                                             .and_then(|body|
                                                 ...
                                                 serde_json::from_str::<models::SessionApiError>(body)
                                                     .map_err(|e| e.into())

                                             ))
                        .map(move |body|
                            GetPlayerProfileResponse::UUIDIsInvalid(body)
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                code => {
